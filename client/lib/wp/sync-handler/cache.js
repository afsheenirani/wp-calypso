...
export class Cache {
	 ...
	 */
	addItem( key ) {
		return this.dropByKey( key ).then( records => {
	...
	clearAll() {
		return localforage.keys().then( keys => {
			...
			return Promise.all( [ ...itemsPromises, recordsPromise ] )
				.then( debugPromise( '%o records removed', syncHandlerKeys.length + 1 ) );
		} );
	}
	...
	pruneRecordsFrom( lifetime = LIFETIME ) {
		...
		return this.dropOlderThan( lifetime ).then( data => {
			...
			return Promise.all( [ ...droppedPromises, recordsListPromise ] )
				.then( debugPromise( '%o records removed', droppedRecords.length + 1 ) );
		} );
	}
}
