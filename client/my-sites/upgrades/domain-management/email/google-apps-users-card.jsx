...
const GoogleAppsUsers = React.createClass( {
	...
	canAddUsers() {
		let domainsInContext = this.props.selectedDomainName ? [ this.props.selectedDomainName ] : this.props.domains.list;
				...
				<CompactCard className="google-apps-users-card">
					<ul className="google-apps-users-card__user-list">
						{ this.props.users.map(
							( user, index ) => <GoogleAppsUserItem key={ index } user={ user } onClick={ this.generateClickHandler( user ) } />
						) }
					</ul>
				</CompactCard>
			</div>
